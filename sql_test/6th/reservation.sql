...
-- {회원정보}
-- 아이디/암호/이름/핸드폰번호/성별/주소/출생일
-- {룸 정보}
-- 룸 넘버/크기/수용인원/요금
-- {예약정보}
-- 고유번호/아이디/예약기간/인원수/결제여부/룸 넘버
...
-- 2. 회원아이디,회원이름,연락처,예약시작일,예약종료일,방번호
select r.userid,
       m.name,
       m.phone,
...
-- 3. 오늘을 포함 예약자 정보(회원이름,연락처,예약시작일,예약종료일,방번호)
select r.userid,
       m.name,
       m.phone,
       to_char(r.sdate,'yyyy-mm-dd') sdate,
...
-- 5. 올해의 총 결제금액 출력
select 
    '올해의 수입 ' || to_char(sum(rm.money),'FM999,999,999')||'원'
from 
